...
p.add_argument('--acc_max', type=float, default=0.004, required=False, help='Acc of the car')
p.add_argument('--collisionR', type=float, default=0.3, required=False, help='Collision radisu between vehicles')
...
dataset = dataio.ReachabilityAir5DSource(numpoints=65000, collisionR=opt.collisionR, 
                                          omega_max=opt.omega_max, acc_max=opt.acc_max, pretrain=opt.pretrain, tMin=opt.tMin,
...
# Define the loss
loss_fn = loss_functions.initialize_hji_air5D(dataset, opt.minWith)
